...
5. On drop:
    ...
    - convert the screen positions to canvas coordinates using React Flow
    - create a new node at that position
    - use an empty label
    - use the defeult node color
...
7. Add a basic renderer for the custom canvas node type so new nodes are visible.
    ...
    For this unit, render every sape as a simple bordered rectangle with the label centered.
